...
const TopicLists = ({grade, searchText}) => {
  ...
  const GET_ITEMS = gql`
  query Grades {
    topics(where: {grade: ${grade.replace(/\s+/g, '')}}) {
      createdBy {
        name
      }
      grade
    ...
    <View style={{ width: "100%", paddingLeft: 10, paddingVertical: 20}}><FlatList
      ...
      renderItem={({ item }) => (
        ...
        <View style={[styles.Contents, styles.elevation] }>
          ...
          <View style={{flexDirection: "row",width: "90%", justifyContent: "space-between", paddingVertical: 10}}>
            <Text>Rating:</Text>
            <Text>95% <AntDesign name="like1" size={15} color={Colors.SECONDARY} /></Text>
          </View>
          </View>
        </TouchableOpacity>
      )}
